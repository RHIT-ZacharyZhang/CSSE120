...
import time
from pip._vendor.requests.models import REDIRECT_STATI
...
#                      (title, (Color, str)
    ...
    def get_next_mouse_click(self):
        # Mark: I don't like rendering here. Students don't see the point of render.
...
#             self.background_color = background_color
    ...
    def _renderShape(self, shape, render_NOW=False):
        ...
        if shape.shape_id_by_canvas[self] is None:
            shape.shape_id_by_canvas[self] = shape._method_for_drawing(self._tkinter_canvas, *coordinates)

        try:
            self._tkinter_canvas.coords(shape.shape_id_by_canvas[self], *coordinates)
        except tkinter.TclError:
            raise Exception("Could not place the shape on the given window. Did you accidentally render a closed window?") from None
...
class Point(_Shape, _ShapeWithOutline):
    ...
    def get_bounding_box(self):
        c1 = Point(self.x - self.width_for_drawing / 2, self.y - self.width_for_drawing / 2)
        c2 = Point(self.x + self.height_for_drawing / 2, self.y + self.height_for_drawing / 2)
        return Rectangle(c1, c2)
...
class Square(_ShapeWithCenter, _ShapeWithOutline):
    ...
    def get_bounding_box(self):
        c1 = Point(self.center.x - self.length_of_each_side / 2, self.center.y - self.length_of_each_side / 2)
        c2 = Point(self.center.x + self.length_of_each_side / 2, self.center.y + self.length_of_each_side / 2)
        return Rectangle(c1, c2)
...
class Color(object):
    """ A Color represents a  fill or outline color created from custom amounts of red, green, and blue light. The arguments are:
    - The RED component (0-255),
    - the GREEN component (0-255),
    - the BLUE component (0-255).
    
    This Color can be passed to RoseGraphics colors such as fill_color and outline_color."""
...
def main():
    ...
    window2 = RoseWindow(500, 300, 'Blue window with yellow canvas',
                         # window_color='blue', # Mark: I assume most students won't use this.
                         canvas_color='yellow')
