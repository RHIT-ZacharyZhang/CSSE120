...
def bullseye(window, colors):
    ...
    # ------------------------------------------------------------------
    h = int(window.height)
    radius_largest = (h / 2) - 25
    center = rg.Point(window.width / 2, window.height / 2)
    delta_radius = (radius_largest - 50) / (len(colors) - 1)
    for k in range(len(colors)):
        circles = rg.Circle(center, radius_largest - delta_radius * k)
        circles.fill_color = colors[k]
