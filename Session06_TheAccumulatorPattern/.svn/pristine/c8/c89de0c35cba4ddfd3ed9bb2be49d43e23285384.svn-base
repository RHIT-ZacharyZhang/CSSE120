...
def draw_bar_chart(height_of_first_bar,
                   height_increment,
                   bar_width,
                   number_of_bars,
                   title):
    ...
    # ------------------------------------------------------------------
    window = rg.RoseWindow(number_of_bars * bar_width + 100, height_of_first_bar + (number_of_bars - 1) * height_increment + 100, title)
    for k in range(number_of_bars):
        corner1 = rg.Point(50 + k * bar_width, 50 + (number_of_bars - 1 - k) * height_increment)
        corner2 = rg.Point(50 + (k + 1) * bar_width, 50 + height_of_first_bar + (number_of_bars - 1) * height_increment)
        bar = rg.Rectangle(corner1, corner2)
        bar.attach_to(window)
    window.render()
