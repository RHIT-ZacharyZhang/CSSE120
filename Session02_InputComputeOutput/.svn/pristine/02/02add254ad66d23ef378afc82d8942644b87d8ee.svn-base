...
def input_angle():
    ...
    (in radians, which is the default units for all the trigonometric
    functions), and prints (each on its own line):
    ...
    print(min(math.sin(angle), math.cos(angle)))



def random_stuff():
    """
    The   random.uniform   function takes two numeric arguments and
    returns a pseudo-random number between the arguments.  For example:
       random1 = random.uniform(-10, 30)
    stores a pseudo-random number between -10 and 30
    in the variable  random1.

    This function (random_stuff) calls the   random.uniform   function
    THREE (3) times to get THREE (3) pseudo-random numbers, where:
      -- The first pseudo-random number is between -10 and 30
      -- The second pseudo-random number is between 0 and 20.
      -- The third pseudo-random number is between -5 and 40.
    ...
    print(min(random1, random2, random3))
    print(max(random1, random2, random3))
